...
public static class OrdersModule
{
    ...
    {
        app.MapGet("/api/orders/user/{userId}", async (Guid userId, IOrderRepository repository) =>
        {
            var orders = await repository.GetByUserIdAsync(userId);
            return Results.Ok(orders);
        });

        app.MapGet("/api/orders/{id}", async (Guid id, IOrderRepository repository) =>
        {
            var order = await repository.GetByIdAsync(id);
            return order is null ? Results.NotFound() : Results.Ok(order);
        });

        app.MapPost("/api/orders", async (Order order, IOrderRepository repository) =>
        {
            var createdOrder = await repository.CreateAsync(order);
            return Results.Created($"/orders/{createdOrder.Id}", createdOrder);
        });

        app.MapPut("/api/orders/{id}", async (Guid id, Order order, IOrderRepository repository) =>
        {
            var updated = await repository.UpdateAsync(order);
            return updated ? Results.Ok() : Results.NotFound();
        });

        app.MapDelete("/api/orders/{id}", async (Guid id, IOrderRepository repository) =>
        {
